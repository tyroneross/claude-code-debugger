...
function banner() {
    line();
    line(`  ${bold('Claude Code Debugger')} ${dim(`v${package_json_1.version}`)}`);
...
function stepHeader(step, total, label, desc) {
...
async function runInit() {
    ...
    const config = (0, config_1.getConfig)({ storageMode: mode });
    ...
    try {
        ...
        line(`  ${RED}x${RESET} Failed: ${err.message}`);
    }
    line();
...
async function runSelfTest(result) {
    const checks = [];
    ...
    checks.push({
        label: 'Core module',
        ok: moduleOk,
        detail: moduleOk ? `v${package_json_1.version}` : 'Run: npm run build',
    });
